...
from app.routers.settings import HUB_CARD_KEYS
...
def test_settings_defaults(client, auth_headers):
    ...
    assert len(data["hub_cards"]) == len(HUB_CARD_KEYS)
    assert {"math", "tutor", "quiz", "timetable"} <= {
        c["key"] for c in data["hub_cards"]
    }
