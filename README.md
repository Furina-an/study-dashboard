...
- 日历与看板：任务支持截止日期，月历视图汇总每日任务 / 复习 / 专注分钟，拖拽看板快速调整状态
- 复习提醒：任务/计划完成后自动生成艾宾浩斯 1/2/4/7/15/30 天复习节点
- 统计可视化：ECharts 专注热力图、30 天趋势、连续专注 streak
- AI 每日学习计划：输入目标 + 截止日期 + 每日可投入分钟，AI 拆成小任务并按天排期自动落库（首页「AI 每日计划」）
- AI 助教（借鉴港大 DeepTutor 辅导模式）：引导式辅导答疑，多会话持久化，支持指定科目与 Markdown 回复；内置「调整窗口」——免费（管理员共享通道）/ 自定义 API 双模式、模型 / 教学风格 / 温度 / 最大回复长度 / 上下文条数，按账号保存默认、对话内可临时覆盖
...
> 📖 完整操作手册见 [docs/操作手册.md](docs/操作手册.md)（安装 / 功能 / 管理后台 / GitHub / 云端部署 / 排障）。
**示例请先看这里**：安装启动、注册邀请码、管理后台、部署上线、常见问题。
## 快速开始
...
- **邀请码**：一次生成 1-50 个；每个码可设「可用次数」（用完自动失效）、「有效天数」（留空=永久）、备注；生成后一键复制；支持启用/停用/删除。
- **用户**：查看全部注册用户与角色，可**禁用/启用**账号——被禁用用户无法登录、已有登录态立即失效（不能禁用自己或管理员）。
- **文件运营**：查看全部用户上传的学习文件，按状态筛选，可下载、放行、拒绝、隔离、重新扫描、标记整合、**推荐全员分享**、删除。
- **安全中心**：一键全量扫描所有用户文件（只读，不修改不删除数据），命中风险仅隔离可放行；查看扫描统计与历史日志。
- **高数资料**：管理「高数复习」页发布的资料文件（上传 / 编辑 / 删除），全员可浏览下载。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/tasks` | 任务列表（`?plan_id=` / `?habit=` / `?due_from=` / `?due_to=`） |
| GET | `/api/calendar?start=&end=` | 日历数据（区间内任务 / 复习节点 / 每日专注分钟） |
| POST | `/api/plans/daily-plan` | AI 每日学习计划（目标 + 截止日期 → 按天排期落库） |
| POST | `/api/tasks` | 新建任务 |
...
| GET | `/api/math/tree` | 高数提纲树（历史数据，已不再展示） |
| GET | `/api/math/resources` | 高数资料列表（全员） |
| POST | `/api/math/resources` | 管理员发布高数资料 |
| GET | `/api/files/recommended` | 管理员推荐的共享资料（全员） |
| POST | `/api/admin/scan-all` | 安全中心：一键全量查杀 |
...
计划状态同上。计划树支持无限层级；「拆解」弹层可选内置模板（学习/项目/备考）或 AI 拆解；「流程图」标签页用 SVG 自绘树状图，可一键导出 SVG / PNG。「AI 每日计划」按目标 + 截止日期调用 LLM 生成任务清单，再按每日可投入分钟排期，任务自动带截止日期；未配置 AI 时该入口返回明确提示，其余功能不受影响。

「日历看板」页（导航「日历」，或首页功能卡片）：月历视图展示每日到期任务、复习节点与专注分钟，点选日期查看当天安排；「看板」标签把任务按待办 / 进行中 / 已完成分三列，支持拖拽更新状态（习惯任务不进入看板）。任务截止日期可在任务页新建 / 行内修改。
